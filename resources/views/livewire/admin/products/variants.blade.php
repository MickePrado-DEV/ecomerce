<div>
        ...
        <header class="bg-[#111827] px-6 py-4 border-b border-gray-700">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Variantes y valores asignados a este producto</p>
                </div>
        ...
        <div class="p-6">
                                ...
                                @foreach ($option->pivot->features as $featureIndex => $feature)
                                    ...
                                    @else
                                        <span
                                            ...
                                            <span class="h-4 w-4 shrink-0 rounded-full border border-gray-900 shadow-inner ring-1 ring-white/10"
                                            ...
                                            <button type="button"
                                                ...
                                                class="inline-flex h-5 w-5 items-center justify-center rounded-full
                                                       text-gray-300/70 opacity-60 group-hover:opacity-100 transition
            ...
            @else
                <div class="flex flex-col items-center justify-center py-12 text-center">
                    <div class="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-gray-800 border border-gray-600">
                        <i class="fa-solid fa-sliders text-xl text-gray-500"></i>
                    </div>
                    <p class="text-sm font-medium text-gray-300">Sin opciones configuradas</p>
                    <p class="text-xs text-gray-500 mt-1 max-w-xs">
                        Agrega tallas, colores u otras variantes para que el cliente pueda elegir al comprar.
                    </p>
                    <button type="button" wire:click="$set('openModal', true)"
                        class="mt-4 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold
    ...
    <x-dialog-modal wire:model.live="openModal">
        ...
        <x-slot name="content">
            <div class="max-h-[60vh] overflow-y-auto px-1 custom-scrollbar">
                <div class="mb-4">
                    ...
                    <x-select wire:model.live="variant.option_id"
                ...
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <span class="text-gray-500 text-xs font-bold uppercase tracking-wider">Valores</span>
                    <hr class="flex-1 border-gray-600">
                </div>
                        ...
                        <li wire:key="variant-feature-{{ $index }}"
                            ...
                            <div class="absolute -top-3 left-4 bg-[#1f2937] px-2">
                                <button type="button" wire:click="removeFeature({{ $index }})"
                                    @disabled(count($variant['features']) <= 1)
                            ...
                            <div>
                                ...
                                <x-select wire:model.live="variant.features.{{ $index }}.id"
                                    wire:change="featureChange({{ $index }})"
                                    class="w-full bg-[#111827] border-gray-600 text-white"
                                    :disabled="empty($variant['option_id'])">
        ...
        <x-slot name="footer">
            <x-button wire:click="$set('openModal', false)" class="bg-gray-700 text-white mr-2">
                Cancelar
            </x-button>
            <x-button wire:click="save" class="bg-blue-600 text-white hover:bg-blue-700">
