<div class="space-y-8">
    {{-- Contenedor Principal en Rejilla (2 columnas en escritorio / 1 en móvil) --}}
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">

        {{-- COLUMNA IZQUIERDA: Opciones asignadas al producto --}}
            ...
            <header class="bg-[#111827] px-6 py-4 border-b border-gray-700">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">Define los atributos de este producto (talla, color,
                            etc.)</p>
                    </div>
            ...
            <div class="p-6">
                            ...
                            <article wire:key="attached-option-{{ $option->id }}"
                                ...
                                <div class="absolute top-4 right-4">
                                    <button type="button"
                                        ...
                                        class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                                ...
                                <div class="flex flex-wrap gap-2 mt-1 min-h-[2rem]">
                                    @foreach ($option->pivot->features as $featureIndex => $feature)
                                        <div
                                            wire:key="option-{{ $option->id }}-feature-{{ $feature['id'] ?? $featureIndex }}">
                                            @if ($option->type == 1)
                                                <span
                                                    class="group inline-flex items-center gap-1.5 bg-gray-800 text-gray-200 text-xs font-medium px-3 py-1.5 rounded-full border border-gray-600 hover:bg-gray-700 transition-colors">
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        class="inline-flex h-4 w-4 items-center justify-center rounded-full opacity-60 group-hover:opacity-100 hover:text-red-400 hover:bg-red-500/20 disabled:opacity-30 disabled:cursor-not-allowed">
                                                        <i class="fa-solid fa-xmark text-[10px]"></i>
                                                    </button>
                                                </span>
                                            @else
                                                <span
                                                    class="group inline-flex items-center gap-2 bg-gray-800 text-gray-200 text-xs font-medium px-2.5 py-1.5 rounded-full border border-gray-600 hover:bg-gray-700 transition-colors">
                                                    <span
                                                    ...
                                                    <button type="button"
                                                        onclick="confirmProductRemovePivotFeature('{{ $this->getId() }}', {{ $option->id }}, {{ $featureIndex }}, {{ count($option->pivot->features) }})"
                                                        @disabled(count($option->pivot->features) <= 1)
                                                        class="inline-flex h-5 w-5 items-center justify-center rounded-full opacity-60 group-hover:opacity-100 hover:text-red-400 hover:bg-red-500/10 disabled:opacity-30 disabled:cursor-not-allowed">
                                                        <i class="fa-solid fa-xmark text-[10px]"></i>
                                                    </button>
                                                </span>
                                            @endif
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p class="text-xs text-gray-500 mt-1 max-w-xs">Agrega opciones antes de crear variantes de
                            venta.</p>
            ...
            <div class="p-6">
                @if ($this->productVariants->isNotEmpty())
                    <div class="space-y-4 max-h-[70vh] overflow-y-auto pr-1 custom-scrollbar">
                        @foreach ($this->productVariants as $itemVariant)
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-wrap gap-2 mt-2">
                                        @foreach ($itemVariant->features as $feature)
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs bg-gray-800 text-gray-300 px-2.5 py-1 rounded-full border border-gray-600">
                                                <span class="text-gray-500">{{ $feature->option->name }}:</span>
                                                @if ($feature->option->type == 2)
                                                    <span class="h-3 w-3 rounded-full border border-gray-700"
                                                        style="background-color: {{ $feature->value }}"></span>
                                                @endif
    ...
    {{-- MODAL: Formulario optimizado en dos filas --}}
    <x-dialog-modal wire:model.live="openModal">
        ...
        <x-slot name="content">
            <div class="max-h-[60vh] overflow-y-auto px-1 custom-scrollbar">
                {{-- FILA 1: Selección de Atributo Madre --}}
                <div class="mb-5 bg-gray-800/40 p-4 rounded-lg border border-gray-700">
                    <x-label class="text-gray-300 mb-1.5 font-semibold">1. Seleccionar Atributo</x-label>
                ...
                <div class="flex items-center gap-4 mb-5">
                    ...
                    <span class="text-gray-500 text-[10px] font-bold uppercase tracking-wider">2. Valores a
                        incluir</span>
                    <hr class="flex-1 border-gray-700">
                </div>

                {{-- FILA 2: Grid multilínea para selectores de valores secundarios --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    @foreach ($optionForm['features'] as $index => $feature)
                        <div wire:key="option-form-feature-{{ $index }}"
                            class="relative rounded-lg border border-gray-600 bg-[#1f2937]/80 p-4 pt-7">

                            <div class="absolute -top-2.5 left-3 bg-[#1f2937] px-2">
                                <button type="button" wire:click="removeOptionFormFeature({{ $index }})"
                                    @disabled(count($optionForm['features']) <= 1)
                                    class="text-red-500 hover:text-red-400 disabled:opacity-40 disabled:cursor-not-allowed text-xs flex items-center gap-1">
                                    <i class="fa-solid fa-trash-can"></i> <span
                                        class="text-[10px] uppercase font-bold">Quitar</span>
                                </button>
                            </div>
                            ...
                            <div>
                                <x-select wire:model.live="optionForm.features.{{ $index }}.id"
                                    ...
                                    class="w-full bg-[#111827] border-gray-600 text-white text-xs" :disabled="empty($optionForm['option_id'])">
                ...
                <div class="flex justify-center pb-2">
                    <button type="button" wire:click="addOptionFormFeature" @disabled(empty($optionForm['option_id']))
                        class="inline-flex items-center gap-2 rounded-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-bold py-2 px-4 transition-colors shadow-sm">
